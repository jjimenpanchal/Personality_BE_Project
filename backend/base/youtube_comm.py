...
class YouTubeApi():
    ...
    def get_video_comments(self, video_id, key, likes_required):
        ...
        commments_list += self.format_comments(results)# jiemn
            ...
            for video_id in list:
                ...
                if comments:
                    for comment in comments:
                        i=0
                        for  temp in range(len(comment)):
                            if comment[temp]==':':
                                i=temp
                                break;
                        temp=comment[i+1::].strip()
